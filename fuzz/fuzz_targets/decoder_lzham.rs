...
// trips the harness.
//
// In this build the LZHAM decoder is header-only: it parses the
// 13-byte `LZH0` framing and either reports the empty-stream success
// path or surfaces `Error::Unsupported` for non-empty payloads. The
// fuzz target still exercises arbitrary-chunked input parsing,
// truncation handling, and the poison-flag pathway.
fuzz_target!(|data: &[u8]| {
    let mut dec = <compcol::lzham::Lzham>::decoder();
