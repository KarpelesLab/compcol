...
// Smoke property: the Reduce decoder must not panic on arbitrary input.
// libfuzzer feeds us garbage bytes; we drive the decoder over them
// and discard the result. Any panic, abort, or undefined behaviour
// trips the harness.
fuzz_target!(|data: &[u8]| {
    let mut dec = <compcol::zip_reduce::ZipReduce>::decoder();
